...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, shuffle=False,num_workers=0,augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix=''):
        super(LoadImagesAndLabels,self).__init__(batch_size=batch_size,shuffle=shuffle,num_workers=num_workers)
        self.img_size = img_size
    ...
    def __getitem__(self, index):
        ...
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img = np.ascontiguousarray(img)
        return jt.array(img), labels_out, self.img_files[index], shapes
    ...
    def collate_fn(batch):
        ...
        return jt.stack(img, 0), jt.contrib.concat(label, 0), path, shapes
    ...
    def collate_fn4(batch):
        ...
        return jt.stack(img4, 0), jt.contrib.concat(label4, 0), path4, shapes4
