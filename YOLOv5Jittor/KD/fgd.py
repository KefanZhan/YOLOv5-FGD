...
class FeatureLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_mask_s = [nn.Conv2d(teacher_channels[i], 1, kernel_size=1) for i in range(len(teacher_channels))]
        self.conv_mask_t = [nn.Conv2d(teacher_channels[i], 1, kernel_size=1) for i in range(len(teacher_channels))]
        ...
        self.channel_add_conv_s = [nn.Sequential(
            ...
            nn.Conv2d(teacher_channels[i] // 2, teacher_channels[i], kernel_size=1)) for i in range(len(teacher_channels))]
        self.channel_add_conv_t = [nn.Sequential(
            nn.Conv2d(teacher_channels[i], teacher_channels[i] // 2, kernel_size=1),
            nn.LayerNorm([teacher_channels[i] // 2, 1, 1]),
            nn.ReLU(),  # yapf: disable
    ...
    def execute(self,
        ...
        tmp = [jt.stack(boxes_list, dim=0) if boxes_list else [] for boxes_list in tmp]
        gt_bboxes = tmp
        ...
        for index in range(len(preds_S)):
            ...
            Mask_fg = jt.zeros_like(S_attention_t)
            Mask_bg = jt.ones_like(S_attention_t)
            wmin,wmax,hmin,hmax = [],[],[],[]
            for i in range(N):
                if (isinstance(gt_bboxes[i], list) and len(gt_bboxes[i]) == 0) or \
                        (isinstance(gt_bboxes[i], jt.Var) and gt_bboxes[i].numel() == 0): # 空 Tensor 或 []
                    # 填充占位符（避免后续索引越界）
                    hmin.append(jt.empty(0, dtype=jt.int32))
                    hmax.append(jt.empty(0, dtype=jt.int32))
                    wmin.append(jt.empty(0, dtype=jt.int32))
                    wmax.append(jt.empty(0, dtype=jt.int32))
                    continue  # 跳过后续处理
                new_boxxes = jt.ones_like(gt_bboxes[i]) # 如果某个批次的图片没有gt_box呢？
                ...
                wmin.append(jt.floor(new_boxxes[:,:, 0]).int())
                wmax.append(jt.ceil(new_boxxes[:,:, 2]).int())
                hmin.append(jt.floor(new_boxxes[:,:, 1]).int())
                hmax.append(jt.ceil(new_boxxes[:,:, 3]).int())

                area = 1.0/(hmax[i].view(1,-1)+1-hmin[i].view(1,-1))/(wmax[i].view(1,-1)+1-wmin[i].view(1,-1))

                for j in range(len(gt_bboxes[i])):
                    Mask_fg[i][hmin[i][j]:hmax[i][j]+1, wmin[i][j]:wmax[i][j]+1] = \
                            jt.maximum(Mask_fg[i][hmin[i][j]:hmax[i][j]+1, wmin[i][j]:wmax[i][j]+1], area[0][j])

                Mask_bg[i] = jt.where(Mask_fg[i]>0, 0, 1)
                if jt.sum(Mask_bg[i]).bool():
                    Mask_bg[i] /= jt.sum(Mask_bg[i])
    ...
    def get_attention(self, preds, temp):
        ...
        value = jt.abs(preds)
        # Bs*W*H
        fea_map = value.mean(dim=1, keepdims=True)
        S_attention = (H * W * nn.softmax((fea_map/temp).view(N,-1), dim=1)).view(N, H, W)

        # Bs*C
        channel_map = value.mean(dim=2,keepdims=False).mean(dim=2,keepdims=False)
        C_attention = C * nn.softmax(channel_map/temp, dim=1)
    ...
    def get_fea_loss(self, preds_S, preds_T, Mask_fg, Mask_bg, C_s, C_t, S_s, S_t):
        ...
        fea_t= jt.mul(preds_T, jt.sqrt(S_t))
        fea_t = jt.mul(fea_t, jt.sqrt(C_t))
        fg_fea_t = jt.mul(fea_t, jt.sqrt(Mask_fg))
        bg_fea_t = jt.mul(fea_t, jt.sqrt(Mask_bg))

        fea_s = jt.mul(preds_S, jt.sqrt(S_t))
        fea_s = jt.mul(fea_s, jt.sqrt(C_t))
        fg_fea_s = jt.mul(fea_s, jt.sqrt(Mask_fg))
        bg_fea_s = jt.mul(fea_s, jt.sqrt(Mask_bg))
    ...
    def get_mask_loss(self, C_s, C_t, S_s, S_t):
        ...
        mask_loss = jt.sum(jt.abs((C_s-C_t)))/len(C_s) + jt.sum(jt.abs((S_s-S_t)))/len(S_s)
