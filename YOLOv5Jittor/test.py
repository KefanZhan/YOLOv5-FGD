...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./configs/yolov5m.yaml', help='source')
    parser.add_argument('--weights',type=str, default='/root/autodl-tmp/project/YOLOv5Jittor/runs/train/exp/weights/best.pkl', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='/root/autodl-tmp/project/NWPU/data.yaml', help='*.data path')
    ...
    parser.add_argument('--project', default='/root/autodl-tmp/project/YOLOv5Jittor/runs/test', help='save to project/name')
    ...
    if opt.task in ['val', 'test']:  # run normally
        ...
        for weights in ['yolov3.pkl', 'yolov3-spp.pkl', 'yolov3-tiny.pkl']:
            f = 'study_%s_%s.txt' % (Path(opt.data).stem, Path(weights).stem)  # filename to save to
            x = list(range(320, 800, 64))  # x axis
            y = []  # y axis
            for i in x:  # img-size
                print('\nRunning %s point %s...' % (f, i))
                r, _, t = test(opt.cfg,opt.data, weights, opt.batch_size, i, opt.conf_thres, opt.iou_thres, opt.save_json,
                               plots=False)
