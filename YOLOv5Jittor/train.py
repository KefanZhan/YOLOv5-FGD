...
def train(hyp, opt, tb_writer=None):
    ...
    save_dir, epochs, batch_size, weights, mode = Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.mode
    ...
    if mode == 'train':
        ...
        print(Fore.BLUE + f"The teacher is: {opt.teacher_weights} .")
    ...
    if mode == 'KD':
        # load teacher model
        teacher = Model(opt.teacher_cfg, ch=3, nc=nc, anchors=hyp.get("anchors"))  # create
        teacher.load(opt.teacher_weights)
        data = jt.randn(2, 3, imgsz, imgsz)
        model.eval()
        teacher.eval()
        ...
        kd_criterion = FeatureLoss(student_channels=s_shapes, teacher_channels=t_shapes)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            for img_idx in range(imgs.shape[0]):  # 遍历 batch 中的每张图片
                ...
                img_target = img_targets[:, 2:] if len(img_targets) > 0 else jt.zeros((0, 4))
                batch['bboxes'].append(img_target)
                for cnt in range(len(img_targets)):
                    batch['batch_idx'].append(img_idx)
                batch['cls'].append(img_targets[:, 0] if len(img_targets) > 0 else jt.zeros(0))
            batch['bboxes'] = jt.contrib.cat(batch['bboxes'], dim=0)
        ...
        if save:
            ...
            jt.save(ema.ema.state_dict(), str(last))
            if best_fitness == fi:
                jt.save(ema.ema.state_dict(), str(best))
        # end epoch ----------------------------------------------------------------------------------------------------
    # end training
    # Strip optimizers
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='/root/autodl-tmp/project/YOLOv5Jittor/configs/yolov5n.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='/root/autodl-tmp/project/NWPU/data.yaml', help='data.yaml path')
    parser.add_argument('--hyp', type=str, default='/root/autodl-tmp/project/YOLOv5Jittor/data/hyp.scratch.yaml', help='hyperparameters path')
    ...
    parser.add_argument('--project', default='/root/autodl-tmp/project/YOLOv5Jittor/runs/train', help='save to project/name')
    parser.add_argument('--name', default='exp', help='save to project/name')
    parser.add_argument('--exist-ok', action='store_true', help='existing project/name ok, do not increment')
    parser.add_argument('--quad', action='store_true', help='quad dataloader')
    # Knowledge Distillation
    parser.add_argument("--mode", default="KD", help="train or KD")
    parser.add_argument("--teacher_cfg", default="/root/autodl-tmp/project/YOLOv5Jittor/configs/yolov5s.yaml", help="train or KD")
    parser.add_argument("--teacher_weights", default="/root/autodl-tmp/project/YOLOv5Jittor/TeacherTraining/runs/train/exp/weights/best.pkl", help="teacher model")
