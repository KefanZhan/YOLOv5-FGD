...
from KD.fgd import FeatureLoss
...
def train(hyp, opt, device, callbacks):
    ...
    Example:
        ...
        - Tutorial: https://docs.ultralytics.com/yolov5/tutorials/train_custom_data
    """
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, mode = (
        Path(opt.save_dir),
        opt.epochs,
        opt.batch_size,
        opt.weights,
    ...
    print(Fore.RED + "-------------------------------------")
    print(Fore.YELLOW + f"Current task: {mode}")
    if mode == 'train':
        print(Fore.GREEN + f"Training model: {opt.cfg}")
    else:
        print(Fore.GREEN + f"KD Method: FGD")
        print(Fore.BLUE + f"The teacher is: {opt.teacher} .")
        print(Fore.BLUE + f"The student is:  {opt.cfg} .")
        print(Fore.GREEN + f"The weight of distillation loss is:  {opt.alpha} .")
    print(Fore.RED + "-------------------------------------")
    ...
    if mode == 'KD':
        # load teacher model
        teacher_weight = opt.teacher
        check_suffix(teacher_weight, ".pt")
        teacher_weight = attempt_download(teacher_weight)
        teacher_ckpt = torch.load(teacher_weight, map_location="cpu")
        teacher = Model(teacher_ckpt["model"].yaml, ch=3, nc=nc, anchors=hyp.get("anchors")).to(device)
        teacher_csd = teacher_ckpt["model"].float().state_dict()
        teacher_csd = intersect_dicts(teacher_csd, teacher.state_dict(), exclude=[])
        teacher.load_state_dict(teacher_csd, strict=False)

        data = torch.randn(2, 3, imgsz, imgsz).to(device)
        model.eval()
        teacher.eval()
        with torch.no_grad():
            _, feat_t = teacher(data, KD=True)
            _, feat_s = model(data, KD=True)
        s_shapes = [feat_s[i].size() for i in range(10, 24)]
        t_shapes = [feat_t[i].size() for i in range(10, 24)]
        s_shapes = [s_shapes[i][1] for i in range(len(s_shapes))]
        t_shapes = [t_shapes[i][1] for i in range(len(t_shapes))]
        kd_criterion = FeatureLoss(student_channels=s_shapes, teacher_channels=t_shapes, device=device).to(device)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            batch = {
                'img': imgs,  # 图像直接使用
                'bboxes': [],  # 初始化目标列表
                'im_file': paths,  # 路径直接使用
                'batch_idx': [],
                'cls':[],
            }
            # 提取batch
            for img_idx in range(imgs.shape[0]):  # 遍历 batch 中的每张图片
                # 提取当前图片的所有目标 (image_idx == img_idx 的目标)
                img_targets = targets[targets[:, 0] == img_idx]
                # 移除 image_idx 列，保留 [class, x, y, w, h]
                img_target = img_targets[:, 2:] if len(img_targets) > 0 else torch.zeros((0, 4))
                batch['bboxes'].append(img_target)
                for cnt in range(len(img_targets)):
                    batch['batch_idx'].append(img_idx)
                batch['cls'].append(img_targets[:, 0] if len(img_targets) > 0 else torch.zeros(0))
            batch['bboxes'] = torch.cat(batch['bboxes'], dim=0)
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                if mode == 'KD':
                    loss *= 2
                    pred_teacher, teacher_features = teacher(imgs, KD=True)
                    pred, features = model(imgs, KD=True)
                    teacher_features = teacher_features[10:24]
                    student_features = features[10:24]
                    kd_loss = opt.alpha * kd_criterion(preds_S=student_features, preds_T=teacher_features, gt_bboxes=batch['bboxes'], img_metas=batch)
                    loss += kd_loss
                if RANK != -1:
...
def parse_opt(known=False):
    ...
    parser.add_argument("--data", type=str, default="/root/autodl-tmp/project/NWPU/data.yaml", help="dataset.yaml path")
    parser.add_argument("--hyp", type=str, default="/root/autodl-tmp/project/YOLOv5Pytorch/data/hyps/hyp.scratch-low.yaml", help="hyperparameters path")
    ...
    parser.add_argument(
        "--evolve_population", type=str, default="/root/autodl-tmp/project/Pytorch/data/hyps", help="location for loading population"
    )
    ...
    parser.add_argument("--project", default="/root/autodl-tmp/project/YOLOv5Pytorch/runs/train", help="save to project/name")
    ...
    parser.add_argument("--mode", default="KD", help="train or KD")
    parser.add_argument("--teacher", default="/root/autodl-tmp/project/YOLOv5Pytorch/TeacherTraining/runs/train/exp/weights/best.pt", help="teacher model")
    parser.add_argument("--alpha", default=0.01, type=float, help="the parameter of distillation loss")
