...
# from YOLOv5Pytorch.visualize import visualize_one_feature
...
class FeatureLoss(nn.Module):
    ...
    def forward(self,
        ...
        for index in range(len(preds_S)):
            ...
            # 可视化FGD的中间结果
            # visualize_one_feature(preds_T[index].mean(dim=1).detach().cpu(), save_dir='/root/autodl-tmp/project/YOLOv5Pytorch/visualize/tea', index=index)
            # visualize_one_feature(preds_S[index].mean(dim=1).detach().cpu(), save_dir='/root/autodl-tmp/project/YOLOv5Pytorch/visualize/stu', index=index)
            # visualize_one_feature(torch.mul(preds_T[index], torch.sqrt(Mask_fg.unsqueeze(1))).mean(dim=1).detach().cpu(), save_dir='/root/autodl-tmp/project/YOLOv5Pytorch/visualize/mul_tea', index=index)
            # visualize_one_feature(torch.mul(preds_S[index], torch.sqrt(Mask_fg.unsqueeze(1))).mean(dim=1).detach().cpu(), save_dir='/root/autodl-tmp/project/YOLOv5Pytorch/visualize/mul_stu', index=index)
